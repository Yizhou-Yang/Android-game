...
public class ShopActivity extends AppCompatActivity implements ShopView {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        presenter = new ShopPresenter(this, db, user);
        presenter.onCreate();
    ...
    public void setInventoryItemOnClickListener() {
        inventoryItemAdapter.setOnItemClickListener(new OnItemClickListener() {
            @Override
            public void onItemClick(DisplayableAdapter.ViewHolder holder, View v, int position) {
                Item item = inventoryItemAdapter.getItem(position);
    ...
    /**
     * Proceed to the next level, ShopExitActivity. Send relevant information such as below to it.
     *
     * @param timeSpent   the amount of time spent in this activity in seconds
     * @param gambleScore score of all the gambles user performed
     * @param exp         the amount of experience user earned in this stage
     */
    @Override
    public void toNextLevel(int timeSpent, double gambleScore, int exp) {
        Intent receivedIntent = getIntent();
        String username = receivedIntent.getStringExtra("username");
